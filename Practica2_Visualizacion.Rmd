...
**Solución:**
- Cargamos el fichero ``weatherAUS.RData`` utilizando lo aprendido en la práctica 1.

```{r Solucion ejercicio 1 apartado 1}
load("data/weatherAUS.RData")
```

- Creamos un objeto ggplot denominado ``grafico1`` en el que esté definido el conjunto de datos ``ds`` alojado en el fichero ``weatherAUS.RData`` y un *aesthetics* en el que en el eje $x$ esté definida la variable ``MinTemp`` y en el eje $y$ esté definida la variable ``MaxTemp``.

```{r Solucion ejercicio 1 apartado 2}
grafico1 <- ggplot(ds, aes(x = MinTemp, y = MaxTemp))
grafico1
```

- Añadimos una capa que defina el objeto geométrico necesario para  construir un gráfico de dispersión de las variables ``MinTemp`` y ``MaxTemp``.

```{r Solucion ejercicio 1 apartado 3}
grafico1 <- grafico1 +geom_point()
grafico1
```

- Generamos el mismo gráfico y lo almacenamos en ``grafico2`` pero definimos la variable categórica ``RainTomorrow`` como el color. 

```{r Solucion ejercicio 1 apartado 4}
grafico2 <- ggplot(ds, aes(x = MinTemp, y = MaxTemp, color = RainTomorrow))
```

A partir del objeto ``grafico2``, representamos un gráfico de dispersión en el que se muestren los puntos con el color del punto correspondiente a la variable ``RainTomorrow``.

```{r Solucion ejercicio 1 apartado 4 parte 2}
grafico2 <- grafico2 + geom_point()
grafico2
```

- Añadimos a ``grafico2`` una capa para añadir un título con **ggtitle()** y lo almacenamos en ``grafico3`.

```{r Solucion ejercicio 1 apartado 5}
grafico3 <- ggplot(ds, aes(x = MinTemp, y = MaxTemp, color = RainTomorrow)) + ggtitle("Relación entre temperaturas máximas y mínimas")

grafico3 <- grafico3 + geom_point()
grafico3
```

- Almacenamos en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio1.pdf``.

```{r Solucion ejercicio 1 apartado 6}
...
```{r Solucion ejercicio 2 apartado 1}
grafico_ej2_1 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am))
grafico_ej2_1
```

- Modificamos el gráfico anterior para incluir información categórica proporcionada por la variable ``RainToday`` con la forma del punto.

```{r Solucion ejercicio 2 apartado 2}
grafico_ej2_2 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday))
grafico_ej2_2 <- grafico_ej2_2 + geom_point()
grafico_ej2_2
```

-  Modificamos el gráfico anterior para incluir información categórica proporcionada por la variable ``RainToday`` con el color del punto. 

```{r Solucion ejercicio 2 apartado 3}
grafico_ej2_3 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, color = RainToday))
grafico_ej2_3 <- grafico_ej2_3 + geom_point()
grafico_ej2_3
```

¿Con qué representación (color o forma) se observa mejor la información que introduce la variable ``RainToday``?

Se ve mucho mejor la representación con diferentes colores en lugar de con diferentes formas.

- Incluimos un título explicativo al gráfico de la figura anterior.

```{r Solucion ejercicio 2 apartado 4}
grafico_ej2_4 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, color = RainToday)) + ggtitle("Relación entre la presión y la temperatura a las 9am")
grafico_ej2_4 <- grafico_ej2_4 + geom_point()
grafico_ej2_4
```

- Almacenamos en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio2.pdf`.

```{r Solucion ejercicio 2 apartado 5}
ggsave(grafico_ej2_4, file = "./figure/Ejercicio2.pdf")
```
...
**Solución:**
- Cargamos el fichero ``iris.tidy.Rdata`` utilizando lo aprendido en la práctica 1.
```{r Solucion ejercicio 3 apartado 1}
load("data/iris_tidy.RData")
```

- Utilizamos la función **str()** para ver qué variables contiene el dataset.
```{r Solucion ejercicio 3 apartado 2}
str(iris.tidy)
```
Vemos que contiene 4 variables: Species, Part, Measure y Value

- Creamos un objeto ggplot denominado ``iris.grafico1`` en el que esté definido el conjunto de datos ``iris.tidy`` alojado en el fichero ``iris.tidy.RData`` y un *aesthetics* en el que en el eje $x$ esté definida la variable ``Species`` y en el eje $y$ esté definida la variable ``Value``.

```{r Solucion ejercicio 3 apartado 3}
iris.grafico1 <- ggplot(iris.tidy, aes(x = Species, y = Value))
iris.grafico1
```

- Añadimos una capa que defina el objeto geométrico necesario para  construir un gráfico de dispersión de las variables ``Species`` y ``Value``.

```{r Solucion ejercicio 3 apartado 4}
iris.grafico1 <- iris.grafico1 + geom_point()
iris.grafico1
```

- Modificamos el *aesthetics* de ``iris.grafico1`` para visualizar  la variable categórica `` Part`` y almacénalo en ``iris.grafico2``.

```{r Solucion ejercicio 3 apartado 5 parte 1}
iris.grafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))
```

A partir del objeto ``iris.grafico2``, representamos un gráfico de dispersión en el que se muestren los puntos dados por las variables

```{r Solucion ejercicio 3 apartado 5 parte 2}
iris.grafico2 <- iris.grafico2 + geom_point()
iris.grafico2
```

- Incluimos la información de la variable ``Measure`` haciendo dos gráficos de dispersión (dispuestos en filas) con **facet_grid()** como en el apartado de introducción para los dos valores que toma la variable ``Measure``. 

```{r Solucion ejercicio 3 apartado 6 parte 1}
iris.grafico31<-ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+geom_point() + facet_grid(Measure~. )
iris.grafico31
```

Cambiamos la disposición de los gráficos para que sea en columnas 

```{r Solucion ejercicio 3 apartado 6 parte 2}
iris.grafico32<-ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+ geom_point() + facet_grid(.~Measure )
iris.grafico32
```


¿Qué disposición, en filas o en columnas, permite comparar mejor los valores de las dos variables?

La disposición en columnas se ve más clara para comprar los valores de las dos variables.

- Cambiamos ahora la capa **geom_point()** del gráfico anterior por la capa  **geom_jitter()**. ¿Qué diferencia observas?

```{r Solucion ejercicio 3 apartado 7}
iris.grafico44 <- iris.grafico2 + geom_jitter()
iris.grafico44

iris.grafico4 <-ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+ geom_jitter() + facet_grid(.~Measure )
iris.grafico4
```

¿Qué diferencia observas?

Con frecuencia hacemos representaciones gráficas en que aparecen muchos datos que se solapan unos sobre otros, lo que no permite visualizar y apreciar la cantidad de datos que se están representando. Podemos mejorar la representación con la opción geom_jitter

En nuestro caso vemos que los valores son desplazados levemente de posición y no están en una linea recta como anteriormente, de manera que el gráfico parece más real y así vemos mejor la distribución.

- Almacenamos en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio3.pdf``.


```{r Solucion ejercicio 3 apartado 8}
ggsave(iris.grafico4, file = "./figure/Ejercicio3.pdf")
```
...
**Solución:**
- Cargamos el fichero ``Ejercicio4_Data.Rdata``. 
```{r Solucion ejercicio 4 apartado 1 parte 1}
load("data/Ejercicio4_Data.Rdata")
```

Examinamos las variables que tiene dicho dataset.

```{r Solucion ejercicio 4 apartado 1 parte 2}
str(DatosEjer4)
```
Vemos que contiene 3 variables: Fecha, Fabricante y Ventas

- Representamos un gráfico de puntos, en el que se muestra la fecha en el eje $x$ y las ventas en el eje $y$. Queremos que los datos de cada fabricante sean de un color distinto. 

```{r Solucion ejercicio 4 apartado 2}
grafico_ejer4 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante)) + geom_point()
grafico_ejer4
```
¿Es este gráfico valido para evaluar la evolución de las ventas de cada fabricante?

Este gráfico no muestra claramente la evolución de las ventas, se ven muchos puntos de dos colores diferentes pero no claramente que representan, por lo tanto, no es un gráfico válido.

- Representamos un gráfico como el anterior en el que se muestre la línea que une los puntos que antes has representado.

```{r Solucion ejercicio 4 apartado 3}
grafico_ejer42 <- grafico_ejer4 + geom_line()
grafico_ejer42
```

- Modificamos el gráfico anterior para que la tendencia de cada fabricante este trazada con un tipo de línea diferente.

```{r Solucion ejercicio 4 apartado 4}
# Para ello añadimos el grupo Fabricante dentro del ggplot y definimos un nuevo aes dentro de geom_line
grafico_ejer42 <-  ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante, group=Fabricante)) + geom_point() + geom_line(aes(linetype = Fabricante))
grafico_ejer42
```

- Añadimos el siguiente título al gráfico *"Ventas mensuales de diferentes compañías tecnológicas"*.

```{r Solucion ejercicio 4 apartado 5}
grafico_ejer43 <- grafico_ejer42 + ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
grafico_ejer43
```

- Almacena en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio4.pdf``.

```{r Solucion ejercicio 4 apartado 6}
ggsave(grafico_ejer43, file = "./figure/Ejercicio4.pdf")
```
...
El argumento **position_dodge(0.05)** hace que los puntos se puedan representar de manera que se esquiven las superposiciones y colocar objetos uno al lado del otro con una distancia de 0.05 en este caso.
...
**Solución:**
- Cargamos el fichero ``Experimento.Rdata``.
```{r Solucion ejercicio 5 apartado 1 parte 1}
load("data/Experimento.Rdata")
```

Examina las variables que tiene dicho dataset.
```{r Solucion ejercicio 5 apartado 1 parte 2}
str(Experimento)
```
Vemos que contiene 4 variables: Complemento, Dosis, Altura y Error_Altura

- Representamos un gráfico de puntos, en la que se muestre la ``Dosis`` en el eje $x$ y la ``Altura`` en el eje $y$. Queremos que los datos de cada ``Complemento`` se represente en una línea diferente con diferentes trazos. 
```{r Solucion ejercicio 5 apartado 2}
grafico_ej5 <- ggplot(Experimento, aes(x = Dosis, y = Altura, linetype = Complemento, shape = Complemento)) + geom_point() + geom_line()
grafico_ej5
```

- Creamos otro gráfico a partir de añadir al gráfico anterior barras de error a cada punto. El error viene dado por la variable ``Error_Altura``.

```{r Solucion ejercicio 5 apartado 3}
grafico_ej52 <- grafico_ej5 + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=0.1, position=position_dodge(0.05)) 
grafico_ej52
```
- Creamos otro gráfico, idéntico al anterior, pero en este caso no puedes incluir el tipo de línea y el tipo de punto en el aes() de la capa ggplot. Además queremos que la paleta de colores sea en tonos de rojos.

```{r Solucion ejercicio 5 apartado 4}
grafico_ej53 <- ggplot(Experimento, aes(x = Dosis, y = Altura, group = Complemento, color = Complemento)) + geom_point(aes(shape = Complemento)) + geom_line(aes(linetype = Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura, linetype = Complemento), width=0.1, position=position_dodge(0.05)) + theme_classic() + scale_color_brewer(palette="Reds")
grafico_ej53
```

- Almacena en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio5.pdf``.

```{r Solucion ejercicio 5 apartado 6}
ggsave(grafico_ej53, file = "./figure/Ejercicio5.pdf")
```
...
- Carga el fichero ``weatherAUS_distributions.RData``.
```{r Solucion ejercicio 6 apartado 1}
load("data/weatherAUS_distributions.RData")
```

- Crea un gráfico en el que estén los boxplots de la variable ``MinTemp`` para cada uno de los años presentes en la variable ``Year`` del dataset.

```{r Solucion ejercicio 6 apartado 2}
grafico_ej6 <- ggplot(ds, aes(x = Year, y = MinTemp)) + geom_boxplot()
grafico_ej6
```
- Crea un gráfico en el que estén los gráficos de violín de la variable ``MaxTemp`` para  cada uno de los años presentes en la variable ``Year`` del dataset.  

```{r Solucion ejercicio 6 apartado 3}
grafico_ej62 <- ggplot(ds, aes(x = Year, y = MaxTemp, fill = Year)) + geom_violin()
grafico_ej62
```

Añade en el mismo gráfico una capa de etiquetas de título (Distribución de temperaturas máximas por año), eje $x$ (Año) y eje $y$ (Temperatura (ºC)).
```{r Solucion ejercicio 6 apartado 3 parte 2}
grafico_ej62 <- grafico_ej62 + labs(title="Distribución de temperaturas máximas por año",x="Año", y = "Temperatura (ºC)")
grafico_ej62
```

- En el gráfico anterior elimina la leyenda y coloca los años en el eje $x$ a 45 grados de inclinación. Busca información en la red sobre cómo hacer esto.

```{r Solucion ejercicio 6 apartado 4}
grafico_ej62 <- grafico_ej62 + theme(legend.position = 'none', axis.text.x = element_text(angle = 45)) 
grafico_ej62
```

- Almacena en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio6.pdf``.
```{r Solucion ejercicio 6 apartado 5}
ggsave(grafico_ej62, file = "./figure/Ejercicio6.pdf")
```
...
**Solución:**
- Descargamos de la url  "http://assets.datacamp.com/blog_assets/chol.txt" el dataset de datacamp que utilizarás para este ejercicio utilizando la función **read.table()**.
```{r Solucion ejercicio 7 apartado 1}
library(readr)
df <- read_table("http://assets.datacamp.com/blog_assets/chol.txt")
```

- Representamos el histograma de la variable ``HEIGHT`` del dataframe cargado del fichero anterior.

```{r Solucion ejercicio 7 apartado 2}
histograma <- ggplot(data=df, aes(x=HEIGHT)) + geom_histogram()
histograma
```

- Añadimos el título ("Histograma de la variable Altura"), una etiqueta en el eje $x$ ("Altura (cm)") y una etiqueta en el eje $y$ ("Frecuencia").

```{r Solucion ejercicio 7 apartado 3}
histograma2 <- histograma + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia")
histograma2
```
- Ajustamos el rango de representación entre 155 y 195, además consiguimos que el histograma tenga 20 *bins*.

```{r Solucion ejercicio 7 apartado 4}
histograma3 <- ggplot(data=df, aes(x=HEIGHT)) + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia") + geom_histogram(bins = 20, breaks=seq(155, 195))
histograma3
```

- Hacemos que las barras del histograma sean de color azul con una grado de transparencia del 50% y el contorno de color rojo.

```{r Solucion ejercicio 7 apartado 5}
histograma4 <- ggplot(data=df, aes(x=HEIGHT)) + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia") + geom_histogram(bins = 20, breaks=seq(155, 195), col="red", fill="blue", alpha = .5)
histograma4
```

- Añadimos la curva de distribución estimada de color verde.
```{r Solucion ejercicio 7 apartado 6}
histograma5 <- ggplot(data=df, aes(x=HEIGHT)) + labs(title="Histograma de la variable Altura", x="Altura (cm)", y="Frecuencia") + geom_histogram(aes(y=stat(density)),bins = 20, breaks=seq(155, 195), col="red", fill="blue", alpha = .5) + geom_density(col="green")
histograma5
```

- Representamos el mismo histograma del apartado anterior para cada valor de la variable ``SMOKE``, recuerda utilizar la función **facet_grid()**.

```{r Solucion ejercicio 7 apartado 7}
histograma6 <- histograma5 +  facet_grid(.~SMOKE)
histograma6
```

- Almacena en formato **pdf** el gráfico resultante en un fichero denominado ``Ejercicio7.pdf``.

```{r Solucion ejercicio 7 apartado 8}
ggsave(histograma6, file = "./figure/Ejercicio7.pdf")
```
